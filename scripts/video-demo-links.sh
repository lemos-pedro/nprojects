...
if [[ -z "$AUTH_BEARER_TOKEN" ]]; then
  echo "AUTH_BEARER_TOKEN is required."
  exit 1
fi
...
HOST_URL="$API_BASE/meetings/$MEETING_ID/demo?userId=$HOST_ID&tenantId=$TENANT_ID&role=host&name=Video%20Host&accessToken=$AUTH_BEARER_TOKEN"
PRESENTER_URL="$API_BASE/meetings/$MEETING_ID/demo?userId=$PRESENTER_ID&tenantId=$TENANT_ID&role=presenter&name=Video%20Presenter&accessToken=$AUTH_BEARER_TOKEN"
...
if [[ -n "$VIEWER_ID" ]]; then
  VIEWER_URL="$API_BASE/meetings/$MEETING_ID/demo?userId=$VIEWER_ID&tenantId=$TENANT_ID&role=viewer&name=Video%20Viewer&accessToken=$AUTH_BEARER_TOKEN"
